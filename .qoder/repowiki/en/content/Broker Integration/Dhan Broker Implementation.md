...
## Introduction
This document explains the DhanBroker implementation that integrates with the Dhan trading platform via the Dhan-Tradehull library. It covers authentication (PIN+TOTP verification, token lifecycle), transport abstraction for REST and WebSocket-based market data, mapping from Dhan wire formats to domain objects, order placement across multiple order types, partial fill handling, idempotent operations, position synchronization, portfolio updates, balance tracking, and resilience patterns such as rate limiting and error recovery tailored to Dhan’s API constraints.

## Project Structure
The Dhan integration is organized into a clear separation of concerns:
- Authentication lifecycle and credential storage
- Transport layer wrapping Tradehull calls with retry and normalization
- Mapper layer converting Dhan responses to domain models
- Broker adapter implementing the broker interface and orchestrating components
...
D["DhanMapper<br/>(dhan_mapper.py)"]
end
subgraph "Auth & Credentials"
E["get_tradehull()<br/>(dhan_auth.py)"]
end
...
## Core Components
- DhanBroker: Implements the BrokerAdapter interface for Dhan, orchestrating auth, transport, and mapping. It handles quote retrieval, historical data, option chains, order placement/cancellation/modification, order/trade books, positions, holdings, and balances.
- DhanAuthProvider: Manages authentication lifecycle, including proactive token refresh using PIN+TOTP when tokens near expiry.
...
The DhanBroker composes three primary subsystems:
- Authentication via DhanAuthProvider and dhan_auth.get_tradehull, which supports shared token store, JWT expiry checks, and PIN+TOTP fallback.
...
- [dhan_auth.py:114-167](file://ntrade/brokers/dhan_auth.py#L114-L167)
- [dhan_transport.py:276-280](file://ntrade/brokers/dhan_transport.py#L276-L280)
...
DhanTransport wraps Tradehull calls with:
- RetryPolicy for transient failures (e.g., LTP fetches)
- Consistent timestamp resolution using injected clock or wall clock
...
- [dhan.py:142-187](file://ntrade/brokers/dhan.py#L142-L187)
...
**Section sources**
- [dhan.py:498-524](file://ntrade/brokers/dhan.py#L498-L524)
- [portfolio.py:63-135](file://ntrade/domain/portfolio.py#L63-L135)
- [portfolio.py:137-173](file://ntrade/domain/portfolio.py#L137-L173)
...
## Conclusion
The DhanBroker implementation provides a robust, layered integration with Dhan’s trading platform. It separates authentication, transport, and mapping concerns while enforcing domain model purity. Resilience patterns like retry policies, proactive token refresh, and timeout-bounded WebSocket reads ensure reliable operation under Dhan’s API constraints. The design supports comprehensive order types, partial fill handling, and accurate portfolio synchronization.
