...
def test_phase_waits_without_absorption():
    k = _kernel()
...
def test_phase_reaches_absorbing_after_absorption_bar():
    k = _kernel()
    strat = ValentiniScalper(symbol=_NIFTY, range_size=4.0, warmup=15)
    k.register_strategy(strat)
    _uptrend_bars(k, 30)                       # base
    _absorption_bar(k, 30, at=115.0)           # phase-1 absorption
    assert strat.phase == "absorbing"


def test_triple_a_long_signal_emits_buy():
    k = _kernel()
    strat = ValentiniScalper(symbol=_NIFTY, range_size=4.0, warmup=15,
                             tp_multiplier=2.0, min_rr=1.5)
    k.register_strategy(strat)
    _uptrend_bars(k, 30)                       # uptrend -> price > VWAP
    _absorption_bar(k, 30, at=115.0)           # absorption (BUY side)
    _candle(k, 31, close=115.4)                # follow-through above VWAP
    _candle(k, 32, close=115.8)                # accumulate -> signal
    fills = _fills(k)
    assert any(f.side == "BUY" for f in fills)


def test_vwap_filter_blocks_signal_when_price_below_vwap():
    k = _kernel()
    strat = ValentiniScalper(symbol=_NIFTY, range_size=4.0, warmup=15)
    k.register_strategy(strat)
...
def test_no_double_entry_while_position_open():
    ...
    _candle(k, 32, close=115.8)                # signal -> BUY (long)
...
def test_stop_loss_exit_emits_sell():
    k = _kernel()
    strat = ValentiniScalper(symbol=_NIFTY, range_size=4.0, warmup=15,
                             tp_multiplier=2.0, min_rr=1.5)
    k.register_strategy(strat)
    ...
    _absorption_bar(k, 30, at=115.0)
    _candle(k, 31, close=115.4)
    _candle(k, 32, close=115.8)                # BUY entry
    assert len(_fills(k)) == 1
    entry = _fills(k)[0]
    # A sharp drop through the stop on the next candle -> SELL exit.
    _candle(k, 33, close=110.0, open_=115.5, high=115.6, low=108.0, volume=500)
    sells = [f for f in _fills(k) if f.side == "SELL"]
    assert len(sells) == 1
    assert sells[0].quantity == entry.quantity


def test_stop_wins_when_bar_hits_both_stop_and_target():
    k = _kernel()
    strat = ValentiniScalper(symbol=_NIFTY, range_size=4.0, warmup=15,
                             tp_multiplier=2.0, min_rr=1.5)
    k.register_strategy(strat)
    _uptrend_bars(k, 30)
    _absorption_bar(k, 30, at=115.0)           # sl = 115 - 4 = 111
    _candle(k, 31, close=115.4)
    _candle(k, 32, close=115.8)                # BUY entry; tp = 125.4
    assert len(_fills(k)) == 1
    # One candle trades through BOTH the stop (111) and the target (125.4):
    # the stop is the protective order and must win (conservative fill).
    _candle(k, 33, close=118.0, open_=116.0, high=126.0, low=110.0)
...
def test_hard_session_close_exits_open_position():
    k = _kernel()
    strat = ValentiniScalper(symbol=_NIFTY, range_size=4.0, warmup=15,
                             session_end="15:25", tp_multiplier=2.0, min_rr=1.5)
    k.register_strategy(strat)
    _uptrend_bars(k, 30)
    _absorption_bar(k, 30, at=115.0)
    _candle(k, 31, close=115.4)
...
def test_extended_chase_beyond_vwap_band_waits_for_pullback():
    k = _kernel()
    strat = ValentiniScalper(symbol=_NIFTY, range_size=4.0, warmup=15,
                             tp_multiplier=2.0, min_rr=1.5)
    k.register_strategy(strat)
...
def test_strategy_can_reenter_after_rejected_entry():
    k = _kernel()
    ...
    _absorption_bar(k, 30, at=115.0)
    _candle(k, 31, close=115.4)
    _candle(k, 32, close=115.8)               # signal -> rejected
    assert _fills(k) == []
    ...
    _absorption_bar(k, 33, at=116.5, volume=2000)
    _candle(k, 34, close=116.9)
    _candle(k, 35, close=117.3)
    assert any(f.side == "BUY" for f in _fills(k))
...
def test_rejected_entry_does_not_arm_phantom_position():
    ...
    _absorption_bar(k, 30, at=115.0)
    _candle(k, 31, close=115.4)
    _candle(k, 32, close=115.8)                # signal -> rejected by risk
    assert strat._active is None
...
def test_absorbing_phase_expires_when_price_runs_away():
    k = _kernel()
    strat = ValentiniScalper(symbol=_NIFTY, range_size=4.0, warmup=15)
    k.register_strategy(strat)
    _uptrend_bars(k, 30)
    _absorption_bar(k, 30, at=115.0)           # absorbing
    assert strat.phase == "absorbing"
    ...
    _absorption_bar(k, 47, at=140.0)
    assert strat.phase == "absorbing"


def test_session_gate_blocks_outside_hours():
    k = _kernel()
    strat = ValentiniScalper(symbol=_NIFTY, range_size=4.0, warmup=15,
                             session_end="15:25", tp_multiplier=2.0, min_rr=1.5)
    k.register_strategy(strat)
    ...
    _absorption_bar(k, 30, at=115.0, ts=datetime(2026, 8, 3, 16, 0))
    _candle(k, 31, close=115.4, ts=datetime(2026, 8, 3, 16, 1))
    _candle(k, 32, close=115.8, ts=datetime(2026, 8, 3, 16, 2))
    assert _fills(k) == []
...
def test_depth_imbalance_filter_blocks_without_buy_pressure():
    k = _kernel()
    strat = ValentiniScalper(symbol=_NIFTY, range_size=4.0, warmup=15,
                             tp_multiplier=2.0, min_rr=1.5,
    ...
    _candle(k, 33, close=116.2)
    assert any(f.side == "BUY" for f in _fills(k))


def test_depth_filter_skipped_without_live_depth():
    k = _kernel()
    strat = ValentiniScalper(symbol=_NIFTY, range_size=4.0, warmup=15,
                             tp_multiplier=2.0, min_rr=1.5,
                             depth_imbalance_min=0.3)
    k.register_strategy(strat)
    _uptrend_bars(k, 30)
    _absorption_bar(k, 30, at=115.0)       # no DepthEvent ever published
    _candle(k, 31, close=115.4)
    _candle(k, 32, close=115.8)
...
def test_position_sized_from_risk_budget():
    k = _kernel()
    strat = ValentiniScalper(symbol=_NIFTY, range_size=4.0, warmup=15,
                             tp_multiplier=2.0, min_rr=1.5,
